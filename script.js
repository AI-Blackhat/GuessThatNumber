...
// enterNumText is a const variable because it won't change.
...
while (restartGame){
  ...
  // Using parseInt to attempt to convert the user's response (string) into a number value
  ...
  while (!rangeNum || rangeNum < 1) {
    ...
    rangeNum = parseInt(rangeNum);
    // For !rangeNum to be true, it must be zero
  }

  // Creates the random number using the range number entered by the user
  randomNum = Math.floor(Math.random() * rangeNum) + 1;
  // Math.floor chops off decimal
  // ++ might also work in place of +1
  ...
  while (true){
    ...
    if (guess === `Mellon`) {
      ...
      guess = prompt(`Please enter a guess from 1 to ${rangeNum}. You have ${attempts} attempt(s) left:`);
      // Note: "Mellon" password can only be used once if it is inside ...
    }
  ...
  while (true){
    ...
    if (playAgain.toUpperCase() === `N`){
      //.toUpperCase allows upper or lowercase
